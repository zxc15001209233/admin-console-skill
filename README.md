...
- 自建侧栏 + 顶栏 + 页签条 + 内容区，浅色 / 深色两套 token，页内切换（记在 `localStorage`）
- 支持从**一句话需求、布局/菜单描述、参考图、需求文档**四种输入生成
- 先产出**单文件 `admin.html`**（mock 假数据 + 筛选/表单/分页等假交互，双击浏览器即可看）
- 页面由需求决定；Skill 只提供积木（列表、抽屉表单、详情、登录含太阳月亮开关与右栏背景、树表、弹窗、批量条等）
...
> 示例：对 AI 说「帮我做一个会议室预约后台原型」，先给【后台方案确认单】，确认后再生成可打开的 `admin.html`（列表、抽屉新建、太阳/月亮切换）。
...
| 2026-09-09 | 主题开关改太阳/月亮；登录左栏加大字、右栏默认可替换背景 |
| 2026-09-10 | 顶栏下多页签：首页不可关、可配缓存；两条分割线；选中只用底边；内容区不再重复页名 |
| 2026-09-09 | 页标题分层、筛选条左对齐、壳图标 SVG、演示角色降级、登录淡圆插画 |
