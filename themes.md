...
| `--fs-page` | `20px` | 空态小标题、工作台数字卡等需要抬一层的字；**页名在页签上，不要再给内容区套 `.page-head h1`** |
...
- **页名**：写在顶栏下的页签上，内容区不要再放 `.page-head h1`。`--fs-page` 留给空态标题、工作台需要抬一层的字，不要和抽屉标题抢同一个 16px。
- **页签**：与顶栏同 `--bg-topbar`；顶栏↔页签、页签↔内容各一条 `inset 0 -1px 0 var(--border)`，禁止删中间那条。选中只用 `--accent-bar` 主色底边，禁止铺 `--bg-page`、禁止圆角卡片。悬停只改字色。
